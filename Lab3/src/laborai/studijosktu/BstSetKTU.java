...
public class BstSetKTU<E extends Comparable<E>> implements SortedSetADT<E>, Cloneable {
    ...
    public SetADT<E> headSet(E element) {
        SetADT<E> subSet = new BstSetKTU();
        for(E e1 : this)
        {
            int cmp = element.compareTo(e1);
            if(cmp > 0)
            {
                subSet.add(e1);
            }
    ...
    private void headSetRecursively(E element, BstNode<E> node, SetADT<E> set)
    {
        if(node == null)
            return;
        int compare = c.compare(element, node.element);
        headSetRecursively(element, node.left, set);
        if(compare > 0)
        {
            set.add(node.element);
            headSetRecursively(element, node.right, set);
        }
    }
    
        @Override
        public int SkaiciuotiLapusRekursyviai()
        {
            return SkaiciuotiLapus(root);
        }
        
        // skaiciuoja lapu skaiciu
        
        private int SkaiciuotiLapus(BstNode<E> node)
        {
            if(node == null)
                return 0;
            if(node.left == null && node.right == null)
                ...
                return SkaiciuotiLapus(node.left) + SkaiciuotiLapus(node.right);
        }
    /**
