...
import laborai.gui.swing.*;
import laborai.gui.*;
import laborai.studijosktu.*;

public class Langas extends JFrame implements ActionListener {

    private static final ResourceBundle MESSAGES = ResourceBundle.getBundle("Lab3.Pranešimai");
    ...
    private SortedSetADTx<Filmas> filmoSet;
    private SetADT<Filmas> naujasFilmas = new BstSetKTU();
    ...
    private void initComponents() {
        ...
        // LangoMeniu juosta patalpinama šiame freime
        setJMenuBar(menus);
    ...
    public void actionPerformed(ActionEvent ae) {
        try {
            ...
            if (source.equals(panButtons.getButtons().get(0))) {
                ...
                int nr = new Random().nextInt(filmoSet.size());
                Filmas film = (Filmas) filmoSet.toArray()[nr];
                KsSwing.oun(taOutput, "Atsitiktinai paimtas filmas iš medžio: "+ film);
                naujasFilmas = filmoSet.headSet(film);
                KsSwing.oun(taOutput, "Medis po headSet() panaudojimo: "+ naujasFilmas);
            }
             else if  (source.equals(panButtons.getButtons().get(6))) {
                KsSwing.oun(taOutput, "Medžio lapų skaičius: "+ filmoSet.SkaiciuotiLapusRekursyviai());
    ...
    public void treeGeneration(String filePath) throws MyException {
        ...
        if (filePath == null) {
            filmoArray = FilmųGamyba.generuotiIrIsmaisyti(sizeOfGenSet, sizeOfInitialSubSet, coef);
            panParam1.getTfOfTable().get(2).setText(String.valueOf(sizeOfLeftSubSet));
        } else { // Skaitoma is failo
            filmoSet.load(filePath);
            filmoArray = new Filmas[filmoSet.size()];
            int i = 0;
            for (Object o : filmoSet.toArray()) {
                filmoArray[i++] = (Filmas) o;
            }
            // Skaitant iš failo išmaišoma standartiniu Collections.shuffle metodu.
            Collections.shuffle(Arrays.asList(filmoArray), new Random());
        }

        // Išmaišyto masyvo elementai surašomi i aibę
        filmoSet.clear();
        for (Filmas a : filmoArray) {
            filmoSet.add(a);
        }
        // Išvedami rezultatai
        // Nustatoma, kad eilutės pradžioje neskaičiuotų išvedamų eilučių skaičiaus
        KsSwing.setFormatStartOfLine(true);
        KsSwing.oun(taOutput, filmoSet.toVisualizedString(delimiter),
    ...
    private void treeAdd() throws MyException {
        Filmas filmo = FilmųGamyba.gautiIsBazes();
        filmoSet.add(filmo);
        panParam1.getTfOfTable().get(2).setText(String.valueOf(--sizeOfLeftSubSet));
        KsSwing.setFormatStartOfLine(true);
        KsSwing.oun(taOutput, filmo, MESSAGES.getString("msg7"));
        KsSwing.oun(taOutput, filmoSet.toVisualizedString(delimiter));
        KsSwing.setFormatStartOfLine(false);
    }
    ...
    private void treeRemove() {
        ...
        if (filmoSet.isEmpty()) {
            ...
            int nr = new Random().nextInt(filmoSet.size());
            Filmas filmo = (Filmas) filmoSet.toArray()[nr];
            filmoSet.remove(filmo);
            KsSwing.oun(taOutput, filmo, MESSAGES.getString("msg6"));
            KsSwing.oun(taOutput, filmoSet.toVisualizedString(delimiter));
        }
        KsSwing.setFormatStartOfLine(false);
    }
    ...
    private void treeIteration() {
        ...
        if (filmoSet.isEmpty()) {
            ...
            KsSwing.oun(taOutput, filmoSet, MESSAGES.getString("msg8"));
        }
        KsSwing.setFormatStartOfLine(false);
    }
    ...
    private void createTree() throws MyException {
        switch (cmbTreeType.getSelectedIndex()) {
            case 0:
                filmoSet = new BstSetKTUx(new Filmas());
                break;
            case 1:
                filmoSet = new AvlSetKTUx(new Filmas());
                break;
            default:
                enableButtons(false);
