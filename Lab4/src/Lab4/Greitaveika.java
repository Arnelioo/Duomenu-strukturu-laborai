...
public class Greitaveika {
    ...
    private final MapKTUx<String, Filmas> autoAtvaizdis
            = new MapKTUx(new String(), new Filmas(), 10, 0.75f, HashType.DIVISION);
    private final MapKTUx<String, Filmas> autoAtvaizdis2
            = new MapKTUx(new String(), new Filmas(), 10, 0.25f, HashType.DIVISION);
    ...
    public void SisteminisTyrimas() throws InterruptedException {
        try {
            ...
            for (int k : TIRIAMI_KIEKIAI) {
                Filmas[] autoArray = FilmųGamyba.gamintiFilmus(k);
                String[] autoIdArray = FilmųGamyba.gamintiFilmuIds(k);
                autoAtvaizdis.clear();
